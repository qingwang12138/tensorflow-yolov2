...
class Train(object):
    def __init__(self, yolo, data):
        ...
        self.learn_rate = tf.train.exponential_decay(self.initial_learn_rate, self.global_step, 2000, 0.1, name='learn_rate')
        ...
        with tf.control_dependencies([self.optimizer]):  # 可以让在这个上下文环境中的操作都在control_inputs 执行
            self.train_op = tf.group(self.average_op)  # 输入输出都是操作
        ...
        # weight_file = 'D:\\reference\\5-dataset\\yolo2_coco_weights\\yolo2_coco.ckpt'
        self.saver.restore(self.sess, weight_file)  # 缺少yolo_v2.ckpt预训练数据
...
def main():
    ...
    parser.add_argument('--weights', default='D:\\reference\\5-dataset\\yolo2_coco_weights\\yolo2_coco.ckpt', type=str)  # darknet-19.ckpt
